...
- ~~Prorrateo por días cuando un inquilino se muda a mitad de periodo~~ — **implementado por completo** (Fases 0-3, 2026-09). El alquiler y las tarifas fijas (agua/gas/mantenimiento) ahora se prorratean por `dias_tramo / dias_periodo` cuando hay más de una ocupación dentro de un mismo período (retiro a mitad de mes, traslado entre unidades, renovación con cambio de precio); la luz sigue su propia lógica de reparto por consumo real. Ver `docs/diseno-ocupaciones-parciales.md` (diseño y 13 decisiones de negocio) y `docs/implementacion-ocupaciones-parciales.md` (qué se construyó, fase por fase, con tests).
